...
4. **Supermemory API:** Using [Supermemory](https://supermemory.ai) as the primary memory layer for infinite context, RAG, and user profiling.
5. **Human-in-the-Loop (HITL) Safety:** Hard-coded safety for destructive actions (Delete/Pay) via Resend approvals.
6. **Native Interaction:** Conversations live where the work happens (Notion comments, Google Doc edits).

## Tech Stack
- **Orchestration:** Temporal (Durable execution).
- **Brain:** Gemini 3 (Flash for vision/fast UI loops, Pro for complex planning).
- **Memory:** Supermemory API (Persistent context and RAG).
- **Communication:** Resend (Email/Webhooks) + Steel (In-app comments).
- **Hands:** Steel (Cloud-based browser automation).
- **Database:** Supabase (Auth, Workspace storage, Job tracking).
- **Backend:** Python (FastAPI).
- **Frontend:** Next.js (Dashboard + Steel "Live View").
